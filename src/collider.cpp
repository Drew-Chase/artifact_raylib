...
namespace artifact
{
    ...
    Collider::Collider(const int x, const int y, const int width, const int height, const std::function<void(Entity *)> &on_entity_overlap) : on_entity_overlap(on_entity_overlap), bounds{static_cast<float>(x), static_cast<float>(y), static_cast<float>(width), static_cast<float>(height)}, is_blocking(false) {}
